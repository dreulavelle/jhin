...
// clone deep-copies a tree so an inlined reference never shares nodes with
// the rule it came from — the copy is re-checked in its own context.
func clone(n node) node {
	...
	case *binaryNode:
		c := *t
		c.l, c.r = clone(t.l), clone(t.r)
		return &c
	case *ternaryNode:
		c := *t
		c.cond, c.then, c.els = clone(t.cond), clone(t.then), clone(t.els)
		return &c
	case *callNode:
		c := *t
		c.args = make([]node, len(t.args))
