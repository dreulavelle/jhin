...
| PTT 1.8.5 (parsett, Python 3.13) | 575.7µs | jhin 10.6x faster on the same corpus (see README divergences) |
| guessit 4.1.0 (Python 3.13) | 4,298µs | jhin 79x faster |

## Read this before quoting the numbers

- **The gold labels were seeded from Python PTT 1.8.5** (`parser/testdata/golden.json`,
  generated by running the actual Python library, then amended where jhin
  deliberately diverges; the README lists those). jhin is contract-tested to
  reproduce the corpus exactly, so **jhin's 100% is by construction** — the
